...
function formatItalianDate(dateValue) {
  if (!dateValue) return '';
  return fromYMD(dateValue).toLocaleDateString('it-IT', {day:'2-digit', month:'2-digit', year:'numeric'});
}
...
if (prevMonthBtn) prevMonthBtn.addEventListener('click', () => { visibleMonth.setMonth(visibleMonth.getMonth() - 1); renderCalendar(); });
if (nextMonthBtn) nextMonthBtn.addEventListener('click', () => { visibleMonth.setMonth(visibleMonth.getMonth() + 1); renderCalendar(); });
if (todayBtn) todayBtn.addEventListener('click', () => { visibleMonth = new Date(today.getFullYear(), today.getMonth(), 1); renderCalendar(); });
...
if (bookingRequestForm) {
  bookingRequestForm.addEventListener('submit', function(event) {
    ...
    const text = ['Ciao, vorrei richiedere disponibilità per A Casa di Marco.','', 'Nome: ' + name, 'Check-in: ' + formatItalianDate(checkin), 'Check-out: ' + formatItalianDate(checkout), 'Ospiti: ' + guests, 'Vi ho trovati tramite: ' + source, message ? 'Richieste: ' + message : ''].filter(Boolean).join('\n');
